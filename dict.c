...
#define DEBUG_DECL , const char *__file__, unsigned __line__
#else
#define DEBUG_PARAMS
#define DEBUG_DECL
#endif
...
struct dictns    *ns;
   ...
   dict = alloc(sizeof(struct dictionary));
    assert(dict);
    ns = alloc(sizeof(struct dictns));
    assert(ns);
    ns->next = 0; 
    ns->first_ns = 0;
    ns->first_key = 0;
...
(struct dictns *ns, void *ptr, unsigned *plen,
